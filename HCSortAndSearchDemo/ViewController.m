...
- (void)viewDidLoad {
    ...
    _dataSource = @[@"搜索栏滚动式通讯录",@"搜索栏固定式通讯录",@"单独调用搜索功能",@"传入Model排序搜索"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch (indexPath.row) {
        ...
        case 3:{
            ...
            ModelViewController *model = [story instantiateViewControllerWithIdentifier:@"Model"];
            [model didSelectedItem:^(NSString *item) {
                cell.detailTextLabel.text = item;
            }];
